...
<script type="text/template" id="story_item_template">
	...
	<div class="leftBottom">
		...
		<% if(obj.status=='completed'){ %>
			...
			<span class="label label-important" rel="tooltip" title="peidetud"><i class="icon-user"></i> <%=votes%></span>
		<% } %>
	</div>
	...
	<div class="rightBottom">
		<span class="label">
			...
			<% if(obj.rate) { %>
				<i rel="tooltip" title="hääletusi" class="icon-user"></i> <%=votes%>
				<i rel="tooltip" title="keskmine: <%=distribution%>" class="icon-fire"></i> <%=rate%>
			<% } %>
		</span>
	</div>
	<div style="clear:both;"></div>
